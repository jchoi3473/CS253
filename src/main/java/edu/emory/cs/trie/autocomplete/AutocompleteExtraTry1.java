...
public class AutocompleteExtraTry1 extends Autocomplete<List<List<String>>> {
    List<String> result ;
    Queue<String> stringQueue ;
    Queue<TrieNode<List<List<String>>>> nodeQueue;
    TrieNode<List<List<String>>> node;

    public AutocompleteExtraTry1(String dict_file, int max) {
        super(dict_file, max);
    }
    ...
    public List<String> getCandidates(String prefix) {
        ...
        if(node.getValue()!=null) {
            ...
            for(int i=0;i<node.getValue().size();i++){
                String resultList = node.getValue().get(i).get(0);
    ...
    */
    ...
    public void pickCandidate(String prefix, String candidate) {
        ...
        if(pickList!=null){
            ...
            for(List<String> tempList : pickList){
                ...
                if(tempList.size()>biggestSize) {
                    biggestSize = tempList.size();
                    sizeIndex = i;
                }
                valueStore.add(tempList);
                i++;
            }
            if(index>=0&&valueStore.get(index).size()==biggestSize)
                sizeIndex=index;

            List<String> temp = valueStore.remove(sizeIndex);
            valueStore.add(0,temp);
        ...
        ///* Return test
        for(int j=0;j<valueStore.size();j++){
            System.out.println("resultList string: "+valueStore.get(j).get(0)+"\t");
        }
        System.out.println();
    }
    ...
    public static void main(String[] args) {
        ...
        AutocompleteExtraTry1 tmp = new AutocompleteExtraTry1(dict_path,30);
