...
public abstract class BucketSort<T extends Comparable<T>> extends AbstractSort<T> {
    ...
    protected void sort(T[] array, int beginIndex, int endIndex, Function<T, T> f) {
        //temp array keeps track of number of elements in each buckets.
        //temp array is adopted in this class because bucket goes away within this method. So numbers needed to be stored before buckets get removed.
        temp = new int[11];
        // add each element in the input array to the corresponding bucket
        for (int i = beginIndex; i < endIndex; i++) {
            buckets.get(getBucketIndex(array[i], f)).add(array[i]);
            temp[getBucketIndex(array[i],f)]++;//whenever a number is getting added into a bucket, temp[i] ++ takes place.
        }
/*
        for(int j = 0;j<10;j++){
